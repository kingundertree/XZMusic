...
@interface XZHeardedViewController ()

@end

@implementation XZHeardedViewController

- (void)viewDidLoad {
    [super viewDidLoad];

    [self setTitleViewWithString:@"听过歌曲"];
...
- (void)initData
{
    self.tableData = (NSMutableArray *)[[XZMusicCoreDataCenter shareInstance] fetchAllMusicByPlayTimeRank];
    [self setTitleViewWithString:[NSString stringWithFormat:@"听过歌曲(%ld)",(long)self.tableData.count]];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if (!cell) {
        ...
        cell.cellType = CellTypeForHeared;
    }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[XZGlobalManager shareInstance].playMusicId isEqualToString:musicInfo.musicId] && [XZGlobalManager shareInstance].isPlaying) {
        ...
        // 设置全局播放数据
        [XZGlobalManager shareInstance].musicArr = (NSMutableArray *)[[XZMusicCoreDataCenter shareInstance] fetchAllMusicByPlayTimeRank];
